...
const Cuenta = () => {
    ...
    const { obtenerCuenta, cuenta, cargando, eliminarCuenta, alerta } = useCuentas();
    ...
    const handleEliminar = async (id) => {
        const alertaEliminar = confirm('¿Deseas Eliminar esta cuenta?');
        if(alertaEliminar){
    ...
    <>
        <div className="flex justify-around">
            <div className=" space-y-2">
                ...
                <p className='font-black text-2xl'><span className='text-indigo-500'>Password: </span>{password}</p>
            </div>

            <div className="flex items-center gap-2 text-gray-400 hover:text-yellow-500 transition-colors">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5}     stroke="currentColor" className="w-6 h-6">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125" />
                      </svg>
                      <Link
                        to={`/cuentas/editar/${params.id}`}
